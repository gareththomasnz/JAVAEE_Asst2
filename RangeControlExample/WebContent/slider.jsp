...
<h2>Struts 2 - Book Database</h2>
	...
	function ajaxUpdate(input,bookId)
	{
		$.ajax({
			  type: "POST",
			  url: "price.action",
			  data: {price: input.value, bookId: bookId},
			}).done(new function(){
				$("#price").val(input.value);				
			});
	}
	</script>
...
<s:form id="updatePrice" action="price.action" method="get" theme="simple"  >
<s:iterator value="books">
	...
	<tr>
		...
		<td>
			<input name="price" type="range" min="0" max="<s:property value="maxPrice"/>" step="1" value="<s:property value="price"/>" onchange="ajaxUpdate(this,<s:property value="bookId"/>);"/>
		</td>
		<td>
